...
if ($conn->connect_error) {
  ...
  $sql = "SELECT m.menuName, m.menuPic, m.menuPrice, IFNULL(SUM(om.quantity), 0) AS totalOrdered
          FROM menu m
          LEFT JOIN ordermenu om ON m.menuId = om.menuId
          GROUP BY m.menuId
  ...
  if ($result && $result->num_rows > 0) {
    ...
    echo '<div class="col-12"><p>No least popular dishes found.</p></div>';
  }
  $conn->close();
}
?>
